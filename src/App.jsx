...
const HierarchicalTree = () => {
  ...
  useEffect(() => {
    ...
    function update(source) {
      ...
      const treeData = tree(root); // tree is a d3.tree layout function
      ...
      const nodeEnter = node
        ...
        .on("mouseenter", function (event, d) {
          if (d.children || d._children) {
            d3.select(this).select("circle").style("fill", "#4f46e5"); // indigo-600 on hover
          }
        })
        .on("mouseleave", function (event, d) {
          d3.select(this)
            .select("circle")
            .style("fill", (d) => (d.children ? "#6366f1" : "#4b5563")); // Revert on mouse leave
        })
        .style("cursor", (d) =>
      ...
      nodeEnter
        ...
        .attr("rx", 8)
        .style("fill", (d) => {
          // A consistent dark fill for all nodes
          return "#1e293b"; // slate-800
        })
        .style("stroke", (d) => {
          // Brighter strokes for contrast on dark background
          if (d.depth === 0) return "#94a3b8"; // slate-400
          if (d.depth === 1) return "#38bdf8"; // sky-400
          if (d.depth === 2) return "#fb923c"; // orange-400
      ...
      nodeEnter
        ...
        .attr("r", 10)
        .style("fill", (d) => (d.children ? "#6366f1" : "#4b5563")) // indigo-500 or gray-600
        .style("stroke", (d) => (d.children ? "#4338ca" : "#374151")) // indigo-700 or gray-700
      ...
      nodeUpdate
        ...
        .transition()
        .duration(duration)
        .style("fill", (d) => (d.children ? "#6366f1" : "#4b5563"));
